...
run(
  ...
  function(run, _, array, $, node, style, attr) {
	...
	array.forEach(["slice", "splice"], function(name){
		var f = ap[name];
	...
	array.forEach(["indexOf", "lastIndexOf", "every", "some"], function(name){
		var f = array[name];
		nlp[name] = function(){ return f.apply(d, [this].concat(aps.call(arguments, 0))); };
	});

	// add conditional methods
        nlp.attr = adaptWithCondition(attr, magicGuard);
        nlp.style = adaptWithCondition(style, magicGuard);
/*
	array.forEach(["attr", "style"], function(name){
		nlp[name] = adaptWithCondition([name], magicGuard);
	});

	// add forEach actions
	array.forEach(["connect", "addClass", "removeClass", "toggleClass", "empty", "removeAttr"], function(name){
	...
	_.extend(NodeList, {
		...
		_place: function(/*Array*/ary, /*DOMNode*/refNode, /*String*/position, /*Boolean*/useClone){
			...
			for(var i = length - 1; i >= 0; i--){
				...
				if(ary._runParse && typeof dojo != "undefined" && dojo.parser && dojo.parser.parse){
					if(!tempNode){
						tempNode = rNode.ownerDocument.createElement("div");
					}
					tempNode.appendChild(n);
					dojo.parser.parse(tempNode);
					n = tempNode.firstChild;
				...
				if(i == length - 1){
					node.place(n, rNode, position);
				}else{
					rNode.parentNode.insertBefore(n, rNode);
				}
				rNode = n;
			}
		},
		...
		*/
		...
		place: function(/*String||Node*/ queryOrNode, /*String*/ position){
			...
			var item = $(queryOrNode)[0];
			return this.forEach(function(n){ node.place(n, item, position); }); // dojo.NodeList
		},
		...
		orphan: function(/*String?*/ simpleFilter){
			...
			return (simpleFilter ? $._filterQueryResult(this, simpleFilter) : this).forEach(orphan); // dojo.NodeList
		},
		...
		adopt: function(/*String||Array||DomNode*/ queryOrListOrNode, /*String?*/ position){
			...
			//		|	"replace"
			// 		or an offset in the childNodes property
			return $(queryOrListOrNode).place(this[0], position)._stash(this);	// dojo.NodeList
		},
		...
		query: function(/*String*/ queryStr){
			...
			var ret = this.map(function(node){
				...
				return $(queryStr, node).filter(function(subNode){ return subNode !== undefined; });
			});
		...
		filter: function(/*String|Function*/ simpleFilter){
			...
			if(typeof simpleFilter == "string"){ // inline'd type check
				items = $._filterQueryResult(this, a[0]);
				if(a.length == 1){
		...
		*/
		...
		at: function(/*===== index =====*/){
			...
			array.forEach(arguments, function(i){ if(this[i]){ t.push(this[i]); }}, this);
